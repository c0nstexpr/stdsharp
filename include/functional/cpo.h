...
namespace stdsharp::functional
{
    ...
    private:
        ...
        } customized_operator_invoke_{};

        static constexpr auto fn_ = ::ranges::overload(tag_invoke_, customized_operator_invoke_);

        template<typename TagT>
        static constexpr auto get_fn(TagT&& tag) noexcept
        {
            return ::ranges::overload(::std::forward<TagT>(tag), fn_);
        }
