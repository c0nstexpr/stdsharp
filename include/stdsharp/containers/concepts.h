...
namespace stdsharp
{
    ...
    namespace details
    {
        ...
        concept iterator_identical =
            requires(std::iterator_traits<T> t_traits, std::iterator_traits<U> u_traits) {
                ...
                requires std::same_as<
                    typename decltype(t_traits)::pointer,
                    typename decltype(u_traits)::pointer>;
                requires std::same_as<
    ...
    concept allocator_aware_container = requires {
        requires requires(
            ...
            requires logical_imply(container_copy_insertable<decltype(instance)>, std::constructible_from<decltype(instance), const decltype(instance)&, decltype(alloc)>);
            requires logical_imply(container_move_insertable<decltype(instance)>, std::constructible_from<decltype(instance), decltype(instance), decltype(alloc)>);
    ...
    concept associative_container = requires {
        requires requires(
            ...
            requires details::container_optional_constructible<
                decltype(instance),
                std::from_range_t,
                decltype(instance)>::template value<decltype(key_cmp), decltype(alloc)>;
#endif
