...
namespace stdsharp::concepts
{
    ...
    template<typename T>
    concept lockable = basic_lockable<T> &&
        requires(T t) //
    { // clang-format off
    ...
    template<typename T>
    concept timed_lockable = lockable<T> &&
        requires(T t) //
    { // clang-format off
    ...
    template<typename T>
    concept shared_timed_lockable = shared_lockable<T> &&
        requires(T t) //
    { // clang-format off
    ...
    template<typename T>
    concept mutex = lockable<T> && !::std::movable<T> && ::std::default_initializable<T> &&
        requires(T t) //
    { // clang-format off
    ...
    template<typename T>
    concept shared_mutex = mutex<T> && shared_lockable<T> &&
        requires(T t) //
    {
